...
import { deleteContainer } from "@/server/services/docker-service";
import { writeAuditLog } from "@/server/services/audit-service";

export async function DELETE(request: Request, { params }: { params: Promise<{ serverId: string; containerId: string }> }) {
  ...
  const authError = authorizeContainerAction({ role: authResult.session.user.role, userId: authResult.session.user.id, action: "delete", container });
  if (authError) return NextResponse.json({ error: authError }, { status: authError === "Forbidden" ? 403 : 400 });

  const parsed = await parseContainerActionRequest(request, container, "delete");
  if ("error" in parsed) return NextResponse.json({ error: parsed.error }, { status: 400 });

  const duplicate = await reserveContainerAction({ idempotencyKey: parsed.data.idempotencyKey, serverId, containerId, action: "delete" });
  ...
  try {
    ...
    await deleteContainer(serverId, containerId);
    await writeAuditLog({
      userId: authResult.session.user.id,
      action: "CONTAINER_DELETE",
      targetType: "container",
