...
import { stopContainer } from "@/server/services/docker-service";
...
export async function POST(request: Request, { params }: { params: Promise<{ serverId: string; containerId: string }> }) {
  ...
  const authError = authorizeContainerAction({ role: authResult.session.user.role, userId: authResult.session.user.id, action: "stop", container });
  if (authError) return NextResponse.json({ error: authError }, { status: authError === "Forbidden" ? 403 : 400 });

  const parsed = await parseContainerActionRequest(request, container, "stop");
  if ("error" in parsed) return NextResponse.json({ error: parsed.error }, { status: 400 });

  const duplicate = await reserveContainerAction({ idempotencyKey: parsed.data.idempotencyKey, serverId, containerId, action: "stop" });
  ...
  try {
    ...
    await stopContainer(serverId, containerId);
    await writeAuditLog({
      userId: authResult.session.user.id,
      action: "CONTAINER_STOP",
      targetType: "container",
    ...
    return NextResponse.json(
      { error: error instanceof Error ? error.message : "Unable to stop container" },
      { status: 502 }
    );
  }
}
